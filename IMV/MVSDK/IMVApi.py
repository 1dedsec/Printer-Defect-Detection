...
# 加载SDK动态库
# load SDK library
MVSDKdll = WinDLL("C:\\Program Files\\HuarayTech\\MV Viewer\\Application\\x64\\MVSDKmd.dll")
...
class MvCamera():
    ...
    def IMV_EnumDevicesByUnicast(pDeviceList, pIpAddress):
        ...
        return MVSDKdll.IMV_EnumDevices(byref(pDeviceList), pIpAddress.encode('ascii'))
    ...
    def IMV_GetFeatureType(self, pFeatureName, pPropertyType):
        MVSDKdll.IMV_GetFeatureType.argtype = (c_void_p, c_void_p, c_uint)
        ...
        return MVSDKdll.IMV_GetFeatureType(self.handle, pFeatureName.encode('ascii'), c_uint(pPropertyType))
    ...
    def IMV_SetEnumFeatureSymbol(self, pFeatureName, pEnumSymbol):
        MVSDKdll.IMV_SetEnumFeatureSymbol.argtype = (c_void_p, c_char_p, c_char_p)
