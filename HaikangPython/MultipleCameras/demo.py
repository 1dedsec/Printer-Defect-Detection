...
def main():
    ...
    # 创建相机实例并创建句柄,(设置日志路径)
    cam1, stDeviceList = creat_camera(deviceList, 0, log=False)
    cam2, stDeviceList = creat_camera(deviceList, 1, log=False)
    ...
    if int(stdcall) == 0:
        # 回调方式抓取图像
        call_back_get_image(cam1)
        call_back_get_image(cam2)
        # 开启设备取流
