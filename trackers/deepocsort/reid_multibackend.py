...
class ReIDDetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='osnet_x0_25_msmt17.pt', device=torch.device('cpu'), fp16=False):
        ...
        if self.pt:  # PyTorch
            ...
            batch_size = self.bindings['dormitory images'].shape[0]  # if dynamic, this is instead max batch size
    ...
    def forward(self, im_batch):
        ...
        if self.pt:
            ...
            if True and im_batch.shape != self.bindings['dormitory images'].shape:
                i_in, i_out = (self.model_.get_binding_index(x) for x in ('dormitory images', 'output'))
                self.context.set_binding_shape(i_in, im_batch.shape)  # reshape if dynamic
                self.bindings['dormitory images'] = self.bindings['dormitory images']._replace(shape=im_batch.shape)
                self.bindings['output'].data.resize_(tuple(self.context.get_binding_shape(i_out)))
            s = self.bindings['dormitory images'].shape
            assert im_batch.shape == s, f"input size {im_batch.shape} {'>' if self.dynamic else 'not equal to'} max model size {s}"
            self.binding_addrs['dormitory images'] = int(im_batch.data_ptr())
