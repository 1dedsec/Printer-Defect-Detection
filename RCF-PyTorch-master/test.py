...
def multi_scale_test(model, test_loader, test_list, save_dir):
    ...
    for idx, image in enumerate(test_loader):
        ...
        contours, hierarchy = cv2.findContours(ms_fuse, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        print(f"img{idx}边缘检测结果:{contours}")
        result = np.zeros_like(ms_fuse)
        # 对每个轮廓进行膨胀和腐蚀操作
        for i, contour in enumerate(contours):
            # 创建一个与原图像大小相同的空白图像
            mask = np.zeros_like(ms_fuse)
        ...
        # 使用cv2.findContours查找result中的轮廓
        contours, hierarchy = cv2.findContours(result, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if len(contours) == 1:
            # 绘制轮廓
            print("只有一个轮廓")
            cv2.drawContours(ms_fuse, contours, -1, (255, 0, 0), thickness=1)
        else:
            print("有多个轮廓")
            max_contour = None
            max_area = 0
            ...
            for contour in contours:
                ...
                if area > max_area:
                    max_area = area
                    max_contour = contour
            cv2.drawContours(ms_fuse, max_contour, -1, (255, 0, 0), thickness=1)
